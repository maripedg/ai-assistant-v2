...
2. **Vector search** — `RetrievalService` queries the alias view named by `embeddings.alias.name` for the top-`k` chunks, or `embeddings.domains.<key>.alias_name` when `X-RAG-Domain` is provided.
...
6. **Response** — Clients receive the question echo, up to three answer fields, `retrieved_chunks_metadata`, `used_chunks`, and a structured `decision_explain` (including `retrieval_target`).
