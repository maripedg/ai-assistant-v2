...
def _split_num_prefix_major(items: List[Dict]) -> List[Dict]:
    ...
    def _item_major(item: Dict) -> str | None:
        meta = item.get("metadata") or {}
        num_prefix = meta.get("num_prefix") or meta.get("numbering_prefix")
        if num_prefix is None:
            return None
        major_part = str(num_prefix).split(".", 1)[0].strip()
        return major_part if major_part.isdigit() else None

    def _is_major_boundary(item: Dict) -> bool:
        meta = item.get("metadata") or {}
        if not _is_heading(item):
            return False
        major = _item_major(item)
        if major is None:
            return False
